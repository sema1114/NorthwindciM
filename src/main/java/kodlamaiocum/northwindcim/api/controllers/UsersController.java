...
public class UsersController {
	...
	@PostMapping("/add")
	public ResponseEntity<?> add(@Valid @RequestBody User user) {//bunu validasyondan geçir!
		
		return ResponseEntity.ok(this.userService.add(user));
	}
	
	
    @ExceptionHandler(MethodArgumentNotValidException.class)                                                //sistemde exception olurşa bunu çağır
    @ResponseStatus(HttpStatus.BAD_REQUEST)
	public ErrorDataResult<Object>handleValidationException(MethodArgumentNotValidException exceptions){
		Map<String,String> validationErrors=new HashMap<String,String>();                                 // (anahtar,hata)email alanı formata uygun değil gibi
